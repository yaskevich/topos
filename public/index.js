...
var apptext = {
    ...
    korona: ['Рэч Паспалітая', 'Commonwealth of Two Nations'],
...
function tiles_load(svg, proj, w, h) {
    ...
    svg.append("g")
        ...
        .attr("xlink:href", function (d) {
            return "//" + ["a", "b", "c"][Math.random() * 3 | 0] + "." + prov + "/" + d.z + "/" + d.x + "/" + d.y + ".png";
        })
        .attr("width", Math.round(tiles.scale))
        .attr("height", Math.round(tiles.scale))
        .attr("x", function (d) { return Math.round((d.x + tiles.translate[0]) * tiles.scale); })
        .attr("y", function (d) { return Math.round((d.y + tiles.translate[1]) * tiles.scale); });
...
function belarus_districts_stat(what2show, color_sceheme_num, path, districts, placeholder, head, board, bar, pattern) {
    ...
    d3.json('/api/districts.json', function (districts_db_data) {
        var domainArray = [];
...
function magdeburg(projection, placeholder, titler, side, bar, template, quick_mode) {
    ...
    var after_anim = 0;
    d3.json("/api/cities.json", function (error, grd) {
        var size = 30;
        var begin_size = size * 2.5;
...
function hexbin_renaming(projection, width, height, svg, titler, board, bar, quick) {
    ...
    d3.json("/api/changes.json", function (error, ren_hg_data) {
...
function viz_renaming(projection, svg, titler, board, bar, quick) {
    ...
    d3.json("/api/changes.json", function (error, grd) {
...
function renderCities(svgObject, proj, cities) {
    // console.log("render cities");
    var bigcities = svgObject
        .selectAll('circle')
        .data(cities)
        .enter()
        .append('g')
...
function renderBorders(svg, path, width, height, projection, raw1, raw2) {
    ...
    outlinegroup
        ...
        .attr('d', path);
    // districts were here
    outlinegroup.append('path').datum(kontur).attr('class', 'boundary').attr('stroke-opacity', 1).attr('d', path);
    setupPatterns(svg, path, kontur);
    return districts;
}
function setupBoard() {
    ...
    svg.call(d3.behavior.drag().on('dragstart', dragstarted).on('drag', dragged).on('dragend', dragended));
    ...
    return [path, height, width, projection, svg, board, head, bar];
};
function setupMap(props, data, data2, data3) {
    var { path, height, width, projection, svg, board, head, bar } = props;
    ...
    $('.navbar-form').submit(function (event) {
        ...
        askdb(svg, head, board, projection, $('#req').val());
    });

    $('.query').click(function () {
        askdb(svg, head, board, projection, queries[$(this).attr('id')], $(this).text());
    });
...
function setupApp() {
    var [path, height, width, projection, svg, board, head, bar] = setupBoard();
    if (fulltestingapp) {
        $("#extendedcontrols").removeClass('hidden');
    } else {
        $('#ModalHowTo').modal('show');
        $("#cit_rel_long").removeClass('hidden');
        $("#basiccontrols").removeClass('hidden');
    }
    d3.queue()
        .defer(d3.json, '/data/btop.json')
        .defer(d3.json, '/data/districts-topo.json')
        .defer(d3.json, '/api/top.json')
        .await(function (_error, data, data2, data3) {
            if (_error) { console.log(_error); }
            setupMap({ path, height, width, projection, svg, board, head, bar }, data, data2, data3);
        });
}
